  ...
  <el-card>
    <el-table border style="margin: 10px 0px;" :data="skuArr">
      ...
      <el-table-column label="操作" width="380px" fixed="right">
        <template #="{row,$index}">
          <el-button type="success" size="small" :icon="row.isSale==1?'Bottom':'Top'" @click="updateSale(row)"></el-button>
  ...
  <!-- 标题部分 -->
   ...
   <template #default>
    ...
    <el-row style="margin: 10px 0px;">
      ...
      <el-col :span="18">
        <el-tag v-for="item in  skuInfo.skuAttrValueList" :key="item.id" style="margin: 5px;">{{ item.saleAttrValueName }}</el-tag>
      </el-col>
    </el-row>
    <el-row style="margin: 10px 0px;">
